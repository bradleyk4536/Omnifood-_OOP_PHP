...
class Block extends Db_Crud {
	...
	protected static $update_sql = "UPDATE block_benefits SET icon=:icon, title=:title, description=:description, display=:display WHERE block_id = :id ";
	...
	public function add_update($control) {
		...
		if($control === "add") :
		$this->section_name	  = static::val_string($_GET['block']);
