...
if(isset($_POST['submit'])) :
	if($hero) :
		...
		if(!empty($_POST['brand_icon'])) :
			...
			//$hero->logo 			 = true;//$hero->set_file($_FILES['logo']);
			//$hero->background_img = $hero->set_file($_FILES['background_img']);
			if(false) :
...
/* BIND INPUTS TO PREPARE STATEMENT BINDPARAMS */
	...
	/*	TEST FOR PREPARE STATEMENT THEN EXECUTE IF TRUE */
				if($hero->add_up_result) :
					...
					$session->message = "<i class='ion-ios-camera'></i> SUCCESS &mdash; NEW MEDIA ADDED <br>" . $hero->file_errors;
				else :
						$session->message = "<i class='ion-sad-outline'></i> FAILURE &mdash; UNABLE TO ADD NEW MEDIA <br>" . $hero->file_errors;
				endif;
			endif;
		endif;
	endif;
				...
				<ol class="breadcrumb">
				 ...
				 <li><i class="ion-ios-camera"></i>&emsp;<a href="../index.php">View Site</a></li>
					...
					<?php if($hero->add_up_result && isset($session->message)) : ?>
							...
							<?php endif; ?>
							<?php if(($hero->add_up_result === false || $hero->media_result === false) && isset($session->message)) : ?>
							<div class="col-sm-6 col-sm-offset-3">
